...
contract Targets is ERC721URIStorage, Ownable {
    ...
    function _transfer(
        ...
        require(_msgSender() == _marketplace, "Kidnapp: transfer can only take place when kidnapped");
        super._transfer(from, to, tokenId);
    }

    function getNewRansomPrice(uint256 oldPrice, uint256 coeff) public pure returns(uint256){
        return coeff * oldPrice; // formule prix 120%
    }

    function mint(address to, string memory name, uint256 startPrice, uint coeff, string memory metadata) public {
        require(_msgSender() == _minter, "Kidnap: only minter can mint Targets");
        require(coeff > 1, "Kidnap: inflation coeff must be greater than 1");
        _counter++;
        super._mint(to, _counter);
        super._setTokenURI(_counter, metadata);
        _targetInfos[_counter] = TargetInfos(name, 1, coeff, getNewRansomPrice(startPrice, coeff), 0, block.timestamp, 0, to);
    }
}